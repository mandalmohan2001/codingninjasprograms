...
class Trie {
   ...
   public:
    ...
    void addAllPossibleWords(TrieNode *root, string pattern, string output) {
        // Base case 
        ...
        for(int i = 0; i < 26; i++) {
            if(root -> children[i] != NULL) {
                char c = 'a' + i;
                addAllPossibleWords(root -> children[i], pattern, output + c);
            }
        }
    }
    ...
    void autoComplete(vector<string> input, string pattern) {
        ...
        TrieNode *node = findWord(root, pattern);
        if(node != NULL) {
            string output = "";
            addAllPossibleWords(node, pattern, output);
        } else {
            return;
        }
    }
