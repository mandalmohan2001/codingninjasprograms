...
void rootToLeafPathsSumToKHelper(BinaryTreeNode<int>* root, int k, string output) {
    if(root == NULL) {
        return;
    }
    ...
    rootToLeafPathsSumToKHelper(root -> left, k - root -> data, output  + to_string(root -> data) + " ");
    rootToLeafPathsSumToKHelper(root -> right, k - root -> data, output + to_string(root -> data) + " ");
}
